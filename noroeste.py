class Noroeste:
    ...
    def default(cls):
        if cls._unica_instancia is None:
            cls._unica_instancia = super(Noroeste, cls).__new__(cls)
        return cls._unica_instancia

    def __init__(self):
        if Noroeste._unica_instancia is not None:
            raise Exception("No se puede crear objeto")

    def caminar(self, unBicho):
        elemento_mapa = unBicho.posicion.forma.no
